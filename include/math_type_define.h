...
namespace DyrosMath
{
	...
	static double lowPassFilterDot(const double input,
								const double prev,
								const double dt,
								const double w)
	{
	...
	template <int N>
	static Eigen::Matrix<double, N, 1> lowPassFilterDot(const Eigen::Matrix<double, N, 1> &input,
													 const Eigen::Matrix<double, N, 1> &prev,
													 const double dt,
													 const double w)
	{
		...
		for (int i = 0; i < N; i++)
		{
			res(i) = lowPassFilterDot(input(i), prev(i), dt, w);
		}
		return res;
	}
