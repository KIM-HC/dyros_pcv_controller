...
int main(int argc, char **argv)
{
	...
	int size_param = yam_["odom_maker_param_list"].size();
	std::cout << "num_file: " << size_file << std::endl;

	for (int f_idx=0; f_idx < size_file; f_idx++) {
		for (int p_idx=0; p_idx < size_param; p_idx++) {
			int file_idx = yam_["odom_maker_file_list"][f_idx].as<int>();
			int param_num = yam_["odom_maker_param_list"][p_idx].as<int>();
			...
			qf.open(q_inp);
			vf.open(v_inp);
			MobileController mc(hz, pkg_path, false, param_num);
			while (!qf.eof())
			{
				// read one tick
				for (int i=0; i<9; i++) {
